...
interface ProjectConfig {
  ...
  agentName: string;
  /** Supabase URL used for direct Realtime subscriptions. Filled in by `connect`. Falls back to HTTP poll if absent. */
  supabaseUrl?: string;
  /** Supabase anon key. Same key the browser receives — safe to persist locally. */
...
function getProject(cfg: Config, code?: string): ProjectConfig {
...
function die(msg: string): never { process.stderr.write(`${C.red}✗ ${msg}${C.reset}\n`); process.exit(1); }
function ok(msg: string) { print(`${C.green}✓${C.reset} ${msg}`); }
function info(msg: string) { print(`${C.gray}  ${msg}${C.reset}`); }
function header(msg: string) { print(`\n${C.bold}${C.cyan}${msg}${C.reset}`); }
function section(msg: string) { print(`\n${C.bold}${msg}${C.reset}`); }
function rule() { print(`${C.gray}${'─'.repeat(56)}${C.reset}`); }

function priorityColor(p: string) {
  if (p === 'urgent')    return `${C.red}紧急${C.reset}`;
  if (p === 'important') return `${C.yellow}重要${C.reset}`;
...
interface SubmissionLite {
  ...
  summary: string;
  created_at: string;
  agent?: { name: string; role_description?: string };
}

// ── commands ──────────────────────────────────────────────────────────────────

async function cmdConnect(positional: string[], flags: Record<string, string>) {
  const code = (positional[0] ?? flags['project'] ?? '').toUpperCase();
  if (!code) die('用法: deeploop connect <PROJECT_CODE> [--name <agent>] [--url <base>]');
  ...
  const agentJoin = await api<{
    projectId: string;
    agentId: string;
  }>(baseUrl, '/api/projects/join', {
    method: 'POST',
  ...
  let panelParticipantId: string | undefined;
  try {
    const panelJoin = await api<{ panelParticipantId: string }>(baseUrl, '/api/projects/join', {
      method: 'POST',
  ...
  try {
    const conn = await api<{ supabaseUrl: string; supabaseAnonKey: string }>(baseUrl, '/api/connection-info');
  ...
  if (panelParticipantId) info(`panelParticipantId: ${panelParticipantId}`);
...
async function cmdBrief(flags: Record<string, string>) {
  const cfg = loadConfig();
  const proj = getProject(cfg, flags['project']);

  const [reqData, subData] = await Promise.all([
    api<{ requirements: Requirement[] }>(proj.url, `/api/requirements?projectId=${proj.projectCode}`),
    api<{ submissions: SubmissionLite[] }>(proj.url, `/api/submissions?projectId=${proj.projectCode}&lite=1`),
  ]);
  ...
  const WEIGHT: Record<string, number> = { urgent: 100, important: 75, normal: 50 };
  const pending = reqs
    .filter(r => !done.has(r.id))
    .sort((a, b) => (WEIGHT[b.priority] ?? 50) - (WEIGHT[a.priority] ?? 50));

  const byPriority = { urgent: 0, important: 0, normal: 0 };
  for (const r of reqs) { const k = r.priority as keyof typeof byPriority; byPriority[k] = (byPriority[k] ?? 0) + 1; }
  ...
  const parts: string[] = [];
  if (byPriority.urgent)    parts.push(`${C.red}紧急 ${byPriority.urgent}${C.reset}`);
  if (byPriority.important) parts.push(`${C.yellow}重要 ${byPriority.important}${C.reset}`);
  if (byPriority.normal)    parts.push(`${C.gray}普通 ${byPriority.normal}${C.reset}`);
  ...
  if (pending.length > 0) {
    ...
    if (pending.length > 5) info(`  …另有 ${pending.length - 5} 条`);
  } else {
...
async function cmdLs(flags: Record<string, string>) {
  ...
  const reqs = reqData.requirements ?? [];
  const subs = subData.submissions ?? [];
  if (reqs.length === 0) { print(`${C.gray}暂无需求${C.reset}`); return; }

  const subCount = (id: string) => subs.filter(s => s.requirement_id === id).length;
...
async function cmdGet(positional: string[], flags: Record<string, string>) {
  const partial = positional[0] ?? flags['req'];
  if (!partial) die('用法: deeploop get <requirement-id>');
...
async function cmdNext(flags: Record<string, string>) {
  const cfg = loadConfig();
  const proj = getProject(cfg, flags['project']);
...
async function cmdLog(flags: Record<string, string>) {
  const cfg   = loadConfig();
  const proj  = getProject(cfg, flags['project']);
  ...
  if (subs.length === 0) { print(`${C.gray}暂无提交记录${C.reset}`); return; }
  ...
  for (const s of subs) {
    ...
    if (s.summary) info(`           ${s.summary.slice(0, 72)}`);
...
async function cmdPush(positional: string[], flags: Record<string, string>) {
  const content = positional[0];
  if (!content) die('用法: deeploop push "<需求内容>" [--priority normal|important|urgent]');
  ...
  const data = await api<{ requirement: { id: string } }>(proj.url, '/api/requirements', {
    method: 'POST',
    body: {
...
async function cmdResult(flags: Record<string, string>) {
  const reqId    = flags['req'];
  const summary  = flags['summary'] ?? '';
  const htmlFile = flags['html'];

  if (!reqId) die('用法: deeploop result --req <id> --summary "<描述>" [--html <文件>]');

  let html = flags['html-inline'] ?? '';
  if (htmlFile) {
    if (!fs.existsSync(htmlFile)) die(`文件不存在: ${htmlFile}`);
    html = fs.readFileSync(htmlFile, 'utf-8');
  }
  if (!html) html = await readStdin();
  if (!html.trim()) die('需要提供 HTML 内容（--html <文件> 或通过 stdin 传入）');
  ...
  const data = await api<{ ok: boolean; submissionId: string }>(proj.url, '/api/submit-result', {
    method: 'POST',
    body: {
      projectId: proj.projectCode,
      requirementId: reqId,
...
const REQ_SECTIONS = ['__REQ__', '__REQ_H__', '__REQ_U__'] as const;
const SECTION_TO_PRIORITY: Record<string, 'normal' | 'important' | 'urgent'> = {
  __REQ__:   'normal',
  __REQ_H__: 'important',
  __REQ_U__: 'urgent',
};

async function cmdWork(flags: Record<string, string>) {
  const cfg  = loadConfig();
  const proj = getProject(cfg, flags['project']);
  ...
  const ping = (requirementId: string, phase: 'picked' | 'done') =>
    api(proj.url, '/api/agent-ping', {
      method: 'POST',
      body: { projectId: proj.projectCode, agentId: proj.agentId, agentName: proj.agentName, requirementId, phase },
    }).catch(() => {});

  const handleRequirement = async (req: { id: string; content: string; priority: 'normal' | 'important' | 'urgent' }) => {
    if (processed.has(req.id) || inflight.has(req.id)) return;
    inflight.add(req.id);
    ...
    try {
      const res = await api<{ submission?: { id: string }; error?: string }>(proj.url, '/api/generate', {
        method: 'POST',
      ...
      process.stderr.write(`${C.gray}[${ts()}]${C.reset} ${C.red}✗ 生成失败: ${(e as Error).message}${C.reset}\n`);
    } finally {
  ...
  if (hasRealtime) {
    ...
    try {
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload: { new: Record<string, unknown> }) => {
            ...
            if (!REQ_SECTIONS.includes(section as typeof REQ_SECTIONS[number])) return;
      ...
      process.stderr.write(`${C.red}realtime 订阅失败：${(e as Error).message}${C.reset}\n`);
  ...
  const t = setInterval(() => void scanBacklog().catch(e => {
    process.stderr.write(`${C.gray}[${ts()}]${C.reset} ${C.red}poll error: ${(e as Error).message}${C.reset}\n`);
  }), 5000);
...
function cmdDisconnect(positional: string[], flags: Record<string, string>) {
  const code = (positional[0] ?? flags['project'] ?? '').toUpperCase();
  const cfg  = loadConfig();

  const target = code || cfg.current;
  if (!target) die('没有当前项目，请指定: deeploop disconnect <CODE>');

  const key = target.toUpperCase();
  if (!cfg.projects[key]) die(`项目 ${key} 不存在`);
  ...
  ok(`已断开项目 ${key}`);
  if (cfg.current) info(`当前项目切换为: ${cfg.current}`);
}

// ── current ───────────────────────────────────────────────────────────────────

async function cmdCurrent(flags: Record<string, string>) {
  const cfg  = loadConfig();
  const proj = getProject(cfg, flags['project']);
  const outFile = flags['out'] ?? flags['o'];

  const data = await api<{
    current: { id: string; html: string; summary: string; agentName: string; createdAt: string; versionNumber: number } | null;
    versionCount: number;
  }>(proj.url, `/api/current-page?projectId=${proj.projectCode}`);
...
async function main() {
  ...
  try {
    if      (cmd === 'connect')                    await cmdConnect(rest, flags);
    else if (cmd === 'brief')                      await cmdBrief(flags);
    else if (cmd === 'ls' || cmd === 'list')       await cmdLs(flags);
    else if (cmd === 'get')                        await cmdGet(rest, flags);
    else if (cmd === 'next')                       await cmdNext(flags);
    else if (cmd === 'log')                        await cmdLog(flags);
    else if (cmd === 'push')                       await cmdPush(rest, flags);
    else if (cmd === 'result' || cmd === 'submit') await cmdResult(flags);
    else if (cmd === 'current')                    await cmdCurrent(flags);
    else if (cmd === 'status')                     await cmdStatus(flags);
    else if (cmd === 'work' || cmd === 'worker')   await cmdWork(flags);
    else if (cmd === 'config')                     cmdConfig();
    else if (cmd === 'disconnect')                 cmdDisconnect(rest, flags);
