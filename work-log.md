...
第十六轮遗留了两个未完成项：`npm run build` 没有跑（”静态复核”），提交没有推。本轮先补跑 build 验证第十四到十六轮所有 writer path 改动，结果 12 条路由全部通过，无 TypeScript 错误。确认第十五、十六轮的 `events/route.ts` 语义校验加固是合法代码。

第二个任务：右侧”板块状态”面板只显示”N 人”数字，看不出哪些角色加入了。这对 demo 演示”6 个视角汇聚”的叙事直接造成障碍，因为观众需要直观感知 Designer、Copywriter、Developer、Product、Marketing、Employee 这 6 个角色的到场情况。
...
在”板块状态”面板标题和可滚动板块卡列表之间插入角色名册区块：

- 从 `@/lib/roles` 导入 `ROLE_IDS`（有序数组）和 `ROLES`（配置字典）。
- 6 个角色 pill 横向排列，圆角胶囊形状。
- 角色已加入：用该角色颜色（如 Designer #A855F7）填充边框和背景，文字用角色色，左侧小圆点同色。
- 角色未加入：边框浅灰、文字 #4b5563，圆点深灰，整体暗淡。
- 下方状态行：`participants.length < 6` 时显示”N / 6 位已加入”（颜色 #374151），全员就绪时切换为”全员就绪 ✓”（#10b981，绿色）。
- 无 Supabase 查询改动，仅使用 page.tsx 已有的 `participants` 状态数组。

#### ✅ npm run build 验证

12 条路由全部通过，第一加载 JS 正常，无 TypeScript 报错。

#### ✅ 提交并推送

```
commit 3d56ff5 — feat: role roster in right panel — shows which of 6 roles have joined
push: main → origin (SSH, <5s)
```

### 为什么这是高价值改动

DeepWork 的 demo 叙事核心是”6 个不同视角汇聚成一个共享工件”。观众如果只看到”2 人”而不知道是哪两个角色，他们无法理解这个多角色协作的意图。角色名册把协议层的”参与者有 role 字段”直接可视化在 UI 上，与 `DeepWorkActor.role` 对齐。

### 验证状态

- ✅ `npm run build` — 12/12 路由通过
- ✅ `git push` — `3d56ff5` 已推到 `main`
- ⬜ 人工确认角色名册在 demo 房间实际渲染正常（建议在配置好 `.env.local` 后测试）

### 下一步建议

1. 用真实 Supabase + Anthropic key 配置 `.env.local`，跑端到端 demo 验证角色名册、合成、归因的真实表现。
2. 执行双机器最小测试：curl Machine A 写 `intent.created`，Machine B 读 `GET /api/workspace` → 写 `patch.proposed`，Machine A 再读 `recentEvents` 验证事件链完整。
